...
import {
  ...
  PlayCircle, Clock, CheckCircle2, Calendar, Zap,
} from "lucide-react";
...
export default async function TeacherDashboard() {
  ...
  const subscription = await getActiveSubscription(userId, "TEACHER");

  const planLabel: Record<string, string> = {
    FREE: "Free", GOLD: "Gold", PLATINUM: "Platinum",
  };
  const planColor: Record<string, string> = {
    FREE:     "text-slate-400 bg-slate-500/10 ring-slate-500/20",
    GOLD:     "text-yellow-400 bg-yellow-500/10 ring-yellow-500/20",
    PLATINUM: "text-cyan-400 bg-cyan-500/10 ring-cyan-500/20",
  };
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              <span className={`rounded-full px-2.5 py-0.5 text-xs font-semibold ring-1 ${planColor[subscription.plan] ?? planColor.FREE}`}>
              ...
              {!subscription.isExpired && subscription.status === "ACTIVE" && subscription.plan !== "FREE" && subscription.currentPeriodEnd && (
